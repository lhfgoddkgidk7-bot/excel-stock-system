...
input_font = Font(name="微软雅黑", size=10, color="1565C0")
input_alignment = Alignment(horizontal="center", vertical="center")

formula_font = Font(name="微软雅黑", size=10, color="000000")
...
def generate_excel():
    ...
    ws_dashboard['B3'].font = Font(name="微软雅黑", size=11, bold=True)
    ws_dashboard['C3'] = "起始日期"
    ws_dashboard['D3'] = datetime(2024, 1, 1).strftime("%Y-%m-%d")
    ...
    ws_dashboard['B5'].font = Font(name="微软雅黑", size=12, bold=True)
    headers_dash = ["指标", "数值", "单位"]
    apply_header(ws_dashboard, headers_dash, start_row=6)

    dash_data = [
        ["总入库金额", "=SUM(入库记录!J:J)", "美元"],
        ["总出库金额", "=SUM(出库记录!I:I)", "美元"],
        ["当前库存价值", "=SUM(期末库存!I:I)", "美元"],
    ...
    for row_idx, data in enumerate(dash_data, start=7):
        ...
        cell_ind.font = Font(name="微软雅黑", size=10, bold=True)
        ...
        cell_unit.font = Font(name="微软雅黑", size=10)
    ...
    x_data = Reference(ws_dashboard, min_col=2, min_row=7, max_row=8)
    y_data = Reference(ws_dashboard, min_col=3, min_row=7, max_row=8)
    chart.add_data(y_data, titles_from_data=False)
    chart.set_categories(x_data)
    ws_dashboard.add_chart(chart, "B15")
    ...
    line_data = Reference(ws_dashboard, min_col=3, min_row=7, max_row=9)
    ...
    for row_idx, data in enumerate(sample_stockin, start=3):
        for col_idx, value in enumerate(data, start=2):
            ...
            if col_idx in [6,7,8,9,10]:
                cell.number_format = '#,##0.00'
    # 修复总成本公式：数量(E) × 长度(F) × 米重(G) × FOB价格(H) × 汇率(I)
    for row in range(3, 103):
        cell = ws_stockin.cell(row=row, column=10)
    ...
    for row_idx, data in enumerate(sample_stockout, start=3):
        for col_idx, value in enumerate(data, start=2):
            ...
            if col_idx in [7,8,9,10]:
                cell.number_format = '#,##0.00'
    # 修复总成本公式：数量(F) × 长度(G) × 米重(H) × VLOOKUP单价 × 7.2
    for row in range(3, 103):
        cell = ws_stockout.cell(row=row, column=9)
        cell.value = f"=IF(AND(F{row}>0,G{row}>0,H{row}>0),F{row}*G{row}*H{row}*VLOOKUP(D{row},商品信息!$B$3:$H$100,6,FALSE)*7.2,0)"
        cell.number_format = '#,##0.00'
        cell.font = formula_font
        cell.alignment = formula_alignment
        cell.border = thin_border
    ...
    for row_idx, data in enumerate(sample_inventory, start=3):
        for col_idx, value in enumerate(data, start=2):
            ...
            if col_idx in [5,6,7,8,9]:
                cell.number_format = '#,##0.00'
    # 修复总成本公式：剩余数量(D) × 长度(E) × 米重(F) × FOB价格(G) × 汇率(H)
    for row in range(3, 103):
        cell = ws_inventory.cell(row=row, column=9)
        cell.value = f"=IF(AND(D{row}>0,E{row}>0,F{row}>0,G{row}>0,H{row}>0),D{row}*E{row}*F{row}*G{row}*H{row},0)"
        cell.number_format = '#,##0.00'
        cell.font = formula_font
        cell.alignment = formula_alignment
        cell.border = thin_border
    ...
    ws_invoice['B2'].font = Font(name="微软雅黑", size=16, bold=True, color="FFFFFF")
    ...
    for row in [4, 5]:
        for col in ['B', 'C', 'F', 'G']:
            ws_invoice[f'{col}{row}'].font = Font(name="微软雅黑", size=10, bold=True)

    # 商品表格标题
    ws_invoice.merge_cells('B7:K7')
    ws_invoice['B7'] = "商品明细 / Detalles de Mercancía"
    ws_invoice['B7'].font = Font(name="微软雅黑", size=12, bold=True, color="FFFFFF")
    ...
    # 总计行
    ws_invoice['K14'].font = Font(name="微软雅黑", size=11, bold=True)
    ...
    ws_invoice['B14'].font = Font(name="微软雅黑", size=11, bold=True)
    ws_invoice['B14'].alignment = Alignment(horizontal="right", vertical="center")

    # 备注
    ws_invoice['B16'] = "备注 / Observaciones:"
    ws_invoice['B16'].font = Font(name="微软雅黑", size=10, bold=True)
    ...
    for row in [21, 22]:
        for col in ['B', 'F']:
            ws_invoice[f'{col}{row}'].font = Font(name="微软雅黑", size=10)
    ...
    guide_content = [
        ...
        ["2. 出入库记录", "录入入库/出库数据，系统自动计算总成本。"],
        ["3. 期末库存", "系统按FIFO自动计算库存价值。"],
        ["4. 仪表盘", "自动汇总财务数据，包含入库/出库金额对比图。"],
    ...
    for row_idx, data in enumerate(guide_content, start=3):
        ...
        cell1.font = Font(name="微软雅黑", size=11, bold=True if data[0] else Font(size=11))
        cell2 = ws_guide.cell(row=row_idx, column=3, value=data[1])
        cell2.font = Font(name="微软雅黑", size=10)
